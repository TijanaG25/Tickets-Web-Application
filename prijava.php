...
<div  class="card p-4 shadow" style="width: 400px; background:#161515; color:white;">
    ...
    <div id="poruka" class="text-center mb-3"></div>
    <form id="loginForm">
      <input type="hidden" name="login" value="1">
        ...
        <button type="submit" name="login" class="btn btn-warning w-100">Prijavi se</button>
    </form>
</div>
</div>
...
$(document).ready(function(){
    $("#loginForm").on("submit", function(e){
        e.preventDefault();
        $.ajax({ url: "login.php",type: "POST",data: $(this).serialize(),dataType: "json",
            success: function(response){
                if(response.status === "uspesno"){
                    $("#poruka").removeClass("text-danger").addClass("text-success").text(response.message);
                    setTimeout(function(){
                        window.location.href = response.redirect;
                    }, 1500);
                } else {
                    $("#poruka").removeClass("text-success").addClass("text-danger").text(response.message);
                }
            },
            error: function(xhr, status, error){
                $("#poruka").addClass("text-danger").text("Greška: " + error);
            }
        });
    });
});
