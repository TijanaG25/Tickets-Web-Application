...
if($vrsta == "Svi događaji") {
    $upit = "SELECT d.* 
              FROM dogadjaji d JOIN vrste v ON d.idVrste = v.id
                                ...
                                WHERE k.naziv = 'Koncert' AND d.status='aktivan' and datum_vreme>now() $uslovGrad
    ...
    $upit = "SELECT d.* 
              FROM dogadjaji d JOIN vrste v ON d.idVrste = v.id
                            ...
                            JOIN sale s ON s.id=d.idSale
                            JOIN adrese a ON a.id=s.idAdrese
              WHERE k.naziv = 'Koncert' AND v.naziv = '$vrsta' AND d.status='aktivan'and datum_vreme>now() $uslovGrad
              ORDER BY d.datum_vreme ASC";
}
